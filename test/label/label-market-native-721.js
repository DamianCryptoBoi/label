...
contract("WyvernExchange", () => {
    beforeEach(async () => {
        ...
        wbnb = new ethers.Contract(WBNB_ADDRESS, WBNB_ABI, accounts[0]);
    ...
    const erc721_for_erc20_test = async (options) => {
        ...
        await Promise.all([
            ...
            wbnb.connect(account_b).approve(payment.address, erc20MintAmount),
        ]);
        await Promise.all([
            erc721.mint(
                account_a.address,
        ...
        const selectorOne = web3.eth.abi.encodeFunctionSignature(
            "ERC721ForNative(bytes,address[7],uint8[2],uint256[6],bytes,bytes)"
        );
        const selectorTwo = web3.eth.abi.encodeFunctionSignature(
            "NativeForERC721(bytes,address[7],uint8[2],uint256[6],bytes,bytes)"
        );

        const paramsOne = web3.eth.abi.encodeParameters(
            ["address[2]", "address", "uint256[2]", "string"],
            [
                [erc721.address, ZERO_ADDRESS],
                payment.address,
