...
contract("WyvernExchange", (accounts) => {
    ...
    const withContracts = async () => {
        ...
        let [atomicizer, erc20, erc721, erc1271, smartContractWallet] =
            await deploy([
                WyvernAtomicizer,
                TestERC20,
                TestERC721,
    ...
    it("allows proxy registration", async () => {
        ...
        assert.isOk(
            await erc721.setApprovalForAll(proxy, true, { from: accounts[6] })
        );
    });
    ...
    it("matches any-any nop order", async () => {
        ...
        const call = {
            ...
            howToCall: 0,
            data: web3.eth.abi.encodeFunctionSignature("test()"),
        };
        assert.isOk(
    ...
    it("matches exactly twice with two-fill", async () => {
        ...
        let [signature1, signature2] = await Promise.all([
    ...
    it("does not match any-any reentrant order", async () => {
        ...
        const exchangec = new web3.eth.Contract(
            exchange.inst.abi,
    ...
    it("matches nft-nft swap order", async () => {
        ...
        const firstData = erc721c.methods
            .transferFrom(accounts[0], accounts[6], nfts[0])
            .encodeABI();
        const secondData = erc721c.methods
            .transferFrom(accounts[6], accounts[0], nfts[1])
            .encodeABI();
    ...
    it("matches two nft + erc20 orders", async () => {
        let { atomicizer, exchange, registry, statici, erc20, erc721 } =
            await withContracts();
        ...
        const firstERC20Call = erc20c.methods
            .transferFrom(accounts[0], accounts[6], 2)
            .encodeABI();
        const firstERC721Call = erc721c.methods
            .transferFrom(accounts[0], accounts[6], nfts[0])
            .encodeABI();
        ...
        const secondERC20Call = erc20c.methods
            .transferFrom(accounts[0], accounts[2], 2)
            .encodeABI();
        const secondERC721Call = erc721c.methods
            .transferFrom(accounts[0], accounts[2], nfts[1])
            .encodeABI();
        ...
        assert.equal(
            await erc20.balanceOf(accounts[6]),
            2,
            "Incorrect balance"
        );
    });
    ...
    it("matches two nft + erc20 orders, real static call", async () => {
        ...
        const firstEDSelector = web3.eth.abi.encodeFunctionSignature(
        ...
        const secondEDSelector = web3.eth.abi.encodeFunctionSignature(
            "transferERC721Exact(bytes,address[7],uint8,uint256[6],bytes)"
        );
        const secondEDParams = web3.eth.abi.encodeParameters(
            ["address", "uint256"],
            [erc721.address, nfts[2]]
        );
        const extradataOneA = web3.eth.abi.encodeParameters(
            ...
            [
                ...
                [
                    (firstEDParams.length - 2) / 2,
                    (secondEDParams.length - 2) / 2,
                ],
        ...
        const firstERC20Call = erc20c.methods
            ...
            .encodeABI();
        const firstERC721Call = erc721c.methods
            .transferFrom(accounts[0], accounts[6], nfts[2])
            .encodeABI();
        const firstData = atomicizerc.methods
            .atomicize(
                [erc20.address, erc721.address],
        ...
        const secondERC721Call = erc721c.methods
            .transferFrom(accounts[6], accounts[0], nfts[0])
            .encodeABI();
        const secondData = atomicizerc.methods
            .atomicize(
                [erc721.address],
    ...
    it("matches erc20-erc20 swap order", async () => {
        ...
        const firstData = erc20c.methods
            .transferFrom(accounts[0], accounts[6], 1)
            .encodeABI();
        const secondData = erc20c.methods
            .transferFrom(accounts[6], accounts[0], 2)
            .encodeABI();
    ...
    it("should match with approvals", async () => {
        ...
        await Promise.all([
            exchange.approveOrder(one, false, { from: accounts[6] }),
            exchange.approveOrder(two, false, { from: accounts[6] }),
        ]);
        const call = {
            target: statici.address,
            howToCall: 0,
    ...
    it("does not match with invalid first order params", async () => {
        ...
        await exchange.inst.setOrderFill_(hashOrder(one), "10", {
            from: accounts[6],
        });
        let [oneSig, twoSig] = await Promise.all([
            exchange.sign(one, accounts[6]),
    ...
    it("does not match with invalid second order params", async () => {
        ...
        await exchange.inst.setOrderFill_(hashOrder(two), "3", {
            from: accounts[6],
        });
        let [oneSig, twoSig] = await Promise.all([
            exchange.sign(one, accounts[6]),
    ...
    it("does not match with nonexistent first proxy", async () => {
        ...
        let [oneSig, twoSig] = await Promise.all([
            exchange.sign(one, accounts[7]),
            exchange.sign(two, accounts[7]),
        ]);
        const call = {
            target: statici.address,
            howToCall: 0,
    ...
    it("allows proxy registration for smart contract", async () => {
        ...
        await smartContractWallet.registerProxy(registry.address, {
            from: accounts[6],
        });
        let proxy = await registry.proxies(smartContractWallet.address);
        assert.isTrue(proxy.length > 0, "No proxy address");
        assert.isOk(
            await smartContractWallet.setApprovalForAll(
                proxy,
                erc721.address,
                true,
                { from: accounts[6] }
    ...
    it("should match with approvals and value to contract", async () => {
        const value = 200;
        let { exchange, registry, statici, smartContractWallet } =
            await withContracts();
        const selector = web3.eth.abi.encodeFunctionSignature(
            "any(bytes,address[7],uint8[2],uint256[6],bytes,bytes)"
        );
        ...
        const two = {
            registry: registry.address,
            maker: smartContractWallet.address,
            staticTarget: statici.address,
            staticSelector: selector,
            staticExtradata: "0x",
            maximumFill: "1",
        ...
        await Promise.all([
            exchange.approveOrder(one, false, { from: accounts[6] }),
            smartContractWallet.approveOrder_(
                exchange.inst.address,
                two.registry,
                two.maker,
                two.staticTarget,
        ...
        assert.isOk(
            await exchange.atomicMatchWith(
                ...
                call,
                ZERO_BYTES32,
                { value: value, from: accounts[6] }
            )
        );
        assert.equal(
            await web3.eth.getBalance(smartContractWallet.address),
            value.toString()
        );
    });
