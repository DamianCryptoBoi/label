...
const OwnableDelegateProxy = artifacts.require(
    "contracts/wyvern/registry/OwnableDelegateProxy.sol:OwnableDelegateProxy"
);
...
contract("WyvernRegistry", (accounts) => {
    it("is deployed", async () => {
        return await WyvernRegistry.new();
    });

    it("does not allow additional grant", async function () {
    ...
    it("has a delegateproxyimpl", async () => {
        ...
        assert.equal(
            delegateproxyimpl.length,
            42,
            "delegateproxyimpl was not set"
        );
    });
    ...
    it("allows proxy to receive tokens before approval", async () => {
        ...
        return assertIsRejected(
            contract.methods
                .receiveApproval(accounts[3], amount, erc20.address, "0x")
    ...
    it("allows proxy to receive tokens", async () => {
        ...
        let erc20 = await TestERC20.new();
        await Promise.all([
            erc20.mint(accounts[3], amount),
            erc20.approve(proxy, amount, { from: accounts[3] }),
        ]);
        let contract = new web3.eth.Contract(AuthenticatedProxy.abi, proxy);
        assert.isOk(
            contract.methods
    ...
    it("returns proxy type", async () => {
        ...
        let contract = new web3.eth.Contract(OwnableDelegateProxy.abi, proxy);
        assert.equal(
            await contract.methods.proxyType().call(),
            2,
    ...
    it("allows proxy ownership transfer", async () => {
        ...
        assert.isOk(
            await contract.methods
                .transferProxyOwnership(accounts[4])
                .send({ from: accounts[3] })
        );
        assert.isOk(
            await contract.methods
                .transferProxyOwnership(accounts[3])
    ...
    it("allows proxy revocation", async () => {
        let [registry, testProxy] = await Promise.all([
            WyvernRegistry.new(),
            TestAuthenticatedProxy.new(),
        ]);
        await registry.registerProxyFor(accounts[1]);
        let proxy = await registry.proxies(accounts[1]);
        ...
        assert.isTrue(
            await contract.methods.revoked().call(),
            "Should be revoked"
        );
        assert.isOk(
            await contract.methods.setRevoke(false).send({ from: accounts[1] }),
            "Should be unrevoked"
        );
    });
    ...
    it("should not allow proxy reinitialization", async () => {
        ...
        return assertIsRejected(
            contract.methods
                .initialize(registry.address, registry.address)
                .send({ from: accounts[1] }),
    ...
    it("allows delegateproxy owner change, but only from owner", async () => {
        ...
        await contract_at.proxyAssert(testProxy.address, 1, call, {
            from: accounts[1],
        });
