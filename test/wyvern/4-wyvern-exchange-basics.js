...
contract("WyvernExchange", (accounts) => {
    ...
    it("validates valid order parameters", async () => {
        ...
        assert.isTrue(
            await exchange.validateOrderParameters(example),
    ...
    it("does not validate authorization by signature with different prefix (personal_sign)", async () => {
        ...
        let hash = hashOrder(example);
        let signature = await wrappedExchange.personalSign(
            example,
            accounts[1]
        );
        assert.isFalse(
            await wrappedExchange.validateOrderAuthorization(
                hash,
                accounts[1],
                signature,
                { from: accounts[5] }
    ...
    it("validates valid authorization by approval", async () => {
        ...
        let valid = await exchange.validateOrderAuthorization(
            hash,
            accounts[0],
